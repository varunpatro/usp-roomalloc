...
def get_solver_package(nrows=None):
    input_data_frame = pd.read_csv('ay1819-usp.csv', nrows=nrows, dtype=object).fillna('')
...
def get_rooms_list_constraint(group_mates, rooms_list):
    '''
    Takes in a list of list of rooms (either a suite or a floor usually) and asserts that all group mates belong to the same room list
    '''
...
PREF0_POINTS = 1
GROUP_POINTS = 10
pref_points = [PREF1_POINTS, PREF2_POINTS, PREF3_POINTS, PREF0_POINTS]
...
def solve():
    ...
    order_weight_to_fill = zip([1, 2, 3, 0], pref_points)
    for order, weight in order_weight_to_fill:
        for person in tqdm(all_people):
            ...
            if pref in pref_comments and pref not in preferences_checked and len(pref_comments[pref]) <= 6:
                ...
                if len(set(map(lambda x: x.room_pref1, group_mates))) == 1:
                    ...
                    if person.room_pref1.is_suite:
                        ...
                        s.add_soft(get_rooms_list_constraint(group_mates, suite_list), GROUP_POINTS)
                    
                    # if the preference is not a suite: same floor
                    else:
                        floor_list = get_floor_list(person.valid_rooms(1))
                        s.add_soft(get_rooms_list_constraint(group_mates, floor_list), GROUP_POINTS)         

            s.add_soft(Or(get_poss_assignments(person, order)), weight)
...
pd.read_csv('ay1819-usp.csv', nrows=11, dtype=object).fillna('')
